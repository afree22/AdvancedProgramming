...
class LinkedList<T> {
    ...
    {
    		int j = 0;
    		ListNode<T> toReturn = null;
    		for(ListNode<T> node=front; node.getNext() != null && j<i; node = node.getNext())
    		{
    			//increment until find the i element, then leave loop
    			toReturn = (ListNode<T>) node.getData();
    		}
